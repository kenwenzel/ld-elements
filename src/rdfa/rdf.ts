...
class FactoryImpl implements Factory {
  ...
  fromTerm(original: Term): any {
    switch (original.termType) {
      case 'BlankNode': return this.blankNode(original.value);
      case 'Quad': throw new Error('Method not implemented.');
      case 'Variable': return this.variable(original.value);
      case 'Literal': return this.literal(original.value, original.language || original.datatype);
      case 'DefaultGraph': return new DefaultGraphImpl();
      case 'NamedNode': return this.namedNode(original.value);
    }
  }

  fromQuad(original: Quad): Quad {
    return this.quad(original.subject, original.predicate, original.object, original.graph);
  }
}
